...
* 字符集就是字符的集合。一般会包含一种语言的字符。比如 GBK，是包含所有

常用汉字字符的字符集。ASCII 是包含英文字符的字符集
...
* ASCII 码，ASCII 表：https://baike.baidu.cnm/item/ASClI/309296#3
* unicode 包含界上所有常用字符，编码也有几种，包括 UTF-8（8-bit

UnicodeTransformationFormat），UTF-16 等。
...
* UTF-16 用 16 个 bit 即两个 byte，这也是 char 占用两个 byte 的原因。当把
...
* ASCII 码 +char，通过 ASCII 表可以找到需要的字符对应的数字·将这个数字转换为 char

然后输出这个 char。ASCII 表：https://baike-baidu.com/item/ASCll/309296#3
...
* 字符串类型的名字叫做 string
* 虽然 String 不是 Java 中的基础类型，但是也可以使用类似的浯法 String str="abc"; 来创建。开始的时候将其当成基础类型，更容易理解
