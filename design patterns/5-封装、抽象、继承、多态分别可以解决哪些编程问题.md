...
下面这段代码是系统中一个简化版的积分模块的代码实现。在电商系统中，我们会给每个用户在消费或参与活动的时候奖励一定的积分，用来鼓励用户在我们的系统中消费
...
从代码中，我们可以发现，Point 类主要有四个属性（也可以叫作成员变量），也就是我们前面定义中提到的信息或者数据。其中，id 表示积分的唯一编号，createTime 表示记录创建的时间，balance 表示积分的余额，balanceLastModifiedTime 表示上次余额变更的时间。

我们参照封装特性，对积分的这四个属性的访问方式进行了限制。调用者只允许通过下面这六个方法来访问或者修改钱包里的数据。
